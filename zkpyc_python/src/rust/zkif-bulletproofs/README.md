...
This is a fork of the [QED-it/bulletproofs](https://github.com/QED-it/bulletproofs) repository (commit c210e4f), which implements a ZK-interface adapter ontop of the Dalek's bulletproofs. A further modification of this implementation is the addition of a `generators_count` argument field, which is to be passed as an unsigned integer up to `2^32`.
...
```bash
zkif cat header.zkif constraints.zkif witness.zkif | zkif_bulletproofs prove 128
zkif cat header.zkif constraints.zkif | zkif_bulletproofs verify 128
```
