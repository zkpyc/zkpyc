...
def sudoku_constraints(grid: Array[Array[int, 9], 9]) -> bool:
    ...
    subgrid_size: int = 3
    for grid_x in range(N // subgrid_size):
        for grid_y in range(N // subgrid_size):
            flat_subgrid: Array[int, 9] = [0 for _ in range(9)]
            idx: int = 0
            for i in range(subgrid_size):
                for j in range(subgrid_size):
                    flat_subgrid[idx] = grid[grid_x * subgrid_size + i][grid_y * subgrid_size + j]
                    idx += 1
            for i in range(3):
                for j in range(i + 1, 3):
                    assert flat_subgrid[i] != flat_subgrid[j], "Each 3x3 sub-grid must contain unique values."
