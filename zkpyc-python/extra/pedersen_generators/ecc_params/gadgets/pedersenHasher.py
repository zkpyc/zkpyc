...
from ..params import Point, PARAM_L
...
class PedersenHasher(object):
    ...
    def __gen_generators(self, c=63):
        ...
        for j in range(0, segments):
            ...
            if j % c == 0:
                current = pedersen_hash_basepoint(name, j // c)
            j = j % c
            if j != 0:
                current = current.double().double().double().double()
            if (current * PARAM_L) != Point.infinity():
                print('ERROR: not on curve')
                print(current * PARAM_L)
    ...
    def __gen_dsl_code(self):
        ...
        segments = len(table)
        program = []
        program.append("{}_table: Array[Array[Array[field, 2], 4], {}] = [".format(self.name, segments))
        for i in range(0, segments):
            r = table[i]
            program.append("  [")
            program.append("    [field({}), field({})],".format(r[0].x, r[0].y))
            program.append("    [field({}), field({})],".format(r[1].x, r[1].y))
            program.append("    [field({}), field({})],".format(r[2].x, r[2].y))
            program.append("    [field({}), field({})]".format(r[3].x, r[3].y))
            if i < segments:
                program.append("  ],")
            else:
                program.append("  ]")
